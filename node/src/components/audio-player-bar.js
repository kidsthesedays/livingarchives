...
class AudioPlayerBar extends Component {
    ...
    render() {
        ...
        const expandIcon: string = this.state.expanded
            ? 'icon ion-ios-arrow-down'
            : 'icon ion-ios-arrow-up'

        const progress: number = this.getDuration() === 0
            ? 0
            : Math.round((elapsedTime / this.audio.duration) * 100)

        if (expanded) {
            const currentTime: string = formatSeconds(elapsedTime)
            const total: string = formatSeconds(this.getDuration())
