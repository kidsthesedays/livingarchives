...
class LocationOverlay extends Component {
    ...
    renderMedia(state: Object, location: Object) {
    ...
    renderAudio(state: Object, location: Object) {
        return (
            <StartAudioButton state={state} location={location} />
        )
    }

    renderVideo(location: Object) {
        const url: string = `/static/videos/${location.meta.video.src}`
    ...
    renderGallery(location: Object) {
        const photos: Array<Object> = location.meta.gallery.photos.map(p => {
            return {
